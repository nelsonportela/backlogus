    ...
    <div class="flex items-center justify-center min-h-screen px-4 py-4">
      ...
      <div
        ...
        <div
          ...
          <div class="flex items-center space-x-3 mb-4 sm:mb-6">
            ...
            <div class="flex-1">
              ...
              <div
                v-if="mediaType === 'book'"
                class="text-sm text-gray-500 dark:text-gray-400 space-y-1">
                <p v-if="item.authors && item.authors.length > 0">
                  By
                  {{
                    item.authors
                      .slice(0, 2)
                  ...
                  <span v-if="item.authors.length > 2">
                    and {{ item.authors.length - 2 }} other{{
                      item.authors.length > 3 ? "s" : ""
                    }}
                  </span>
                </p>
                <div
                  v-if="item.rating || item.pages"
                  ...
                  <span v-if="item.pages">
                    {{ item.pages.toLocaleString() }} pages
                  </span>
                </div>
                <p v-if="item.series_names && item.series_names.length > 0">
          ...
          <!-- Current Season/Episode (for TV shows) -->
          <div v-if="mediaType === 'show'" class="grid grid-cols-2 gap-3">
            <div>
              <label
                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Current Season
              ...
              <input
                v-model.number="selectedCurrentSeason"
                type="number"
                min="1"
                :max="item?.seasons || 999"
                class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-primary-500 focus:border-primary-500 dark:focus:ring-primary-400 dark:focus:border-primary-400"
                placeholder="1" />
            </div>
            <div>
              ...
              <input
                v-model.number="selectedCurrentEpisode"
                type="number"
                min="1"
                class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-primary-500 focus:border-primary-500 dark:focus:ring-primary-400 dark:focus:border-primary-400"
                placeholder="1" />
...
// Form state
const selectedStatus = ref("BACKLOG");
const selectedReview = ref(null);
const selectedPlatform = ref("");
const selectedNotes = ref("");
const selectedCurrentSeason = ref(1);
const selectedCurrentEpisode = ref(1);
...
watch(
  ...
  (newVal) => {
    if (newVal) {
      // Set default status - BACKLOG is universal for all media types
      selectedStatus.value = "BACKLOG";
      selectedReview.value = null;
      selectedPlatform.value = "";
      selectedNotes.value = "";
      selectedCurrentSeason.value = 1;
      selectedCurrentEpisode.value = 1;
