    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <div 
        ...
        <div class="flex items-center mb-2">
          ...
          <span class="font-medium text-green-800 dark:text-green-200">{{ getContinueSectionTitle() }}</span>
        </div>
        ...
        <span 
          v-else
          class="text-green-500 dark:text-green-400 text-sm italic">
      ...
      <div 
        ...
        <div class="flex items-center mb-2">
          ...
          <span class="font-medium text-yellow-800 dark:text-yellow-200">Tackle Your Backlog</span>
        </div>
        <p class="text-sm text-yellow-700 dark:text-yellow-300 mb-2">
          {{ backlogCount }} item{{ backlogCount !== 1 ? 's' : '' }} waiting for you
        </p>
        <button 
      ...
      <div class="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-lg p-4 border border-blue-200 dark:border-blue-700">
        <div class="flex items-center mb-2">
          ...
          <span class="font-medium text-blue-800 dark:text-blue-200">Discover New</span>
        </div>
...
const emit = defineEmits(['continue-randomly', 'tackle-backlog', 'navigate-to-discover']);

const { getMediaConfig, formatRelativeTime } = useMediaTypes();
...
const currentlyPlaying = computed(() => {
  ...
  if (props.activeMediaType === "games") {
    ...
    return (currentStats.recentActivity || []).filter(activity => 
      activity.status === "playing"
    );
  } else if (props.activeMediaType === "movies" || props.activeMediaType === "shows") {
...
const backlogCount = computed(() => {
  ...
  if (props.activeMediaType === "all") {
    ...
    return (gameStats.want_to_play || 0) + 
           (movieStats.want_to_watch || 0) + 
           (showStats.want_to_watch || 0);
  } else {
    const statusDist = currentStats.statusDistribution || {};
    return statusDist.want_to_play || statusDist.want_to_watch || statusDist.want_to_read || 0;
  }
});
